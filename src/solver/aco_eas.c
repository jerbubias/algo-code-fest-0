...
/* Ant Colony Optimisation - Elitist Ant System */
...
int main(int argc, char **argv)
{
    ...
    double ph_val, ph_wt, cost_wt, evp_rate, Q, e, cost, auxcost, mincost = DBL_MAX, *ph, *enumDes, *phRate, sum;

    if (argc < 10) {
        fprintf(stderr, "Usage: %s <file name> <pop size> <ph val> <ph wt> <cost wt> <evp rate> <Q> <e> <max iter>\n", argv[0]);
        return 0;
    }
    ...
    if (pop_size < 1) {
        fprintf(stderr, "Invalid population size. Should be a positive value.\n"
                        "Usage: %s <file name> <pop size> <ph val> <ph wt> <cost wt> <evp rate> <Q> <e> <max iter>\n",
                argv[0]);
        return 0;
    }
    if (ph_val < 0.0) {
    ...
    if (e <= 0.0) {
        fprintf(stderr, "Invalid weight for elitist strategy. Should be a positive value.\n"
                        "Usage: %s <file name> <pop size> <ph val> <ph wt> <cost wt> <evp rate> <Q> <e> <max iter>\n",
                argv[0]);
        return 0;
    }
    ...
    if (p != NULL) {
        ...
        for (i = 0; i < max_iter; ++i) {
            ...
            /* Pheromone update */
            for (j = 0; j < m; ++j)
                ph[j] = (1.0 - evp_rate) * ph[j] + phRate[j];
        }
